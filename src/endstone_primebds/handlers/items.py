...
def handle_item_pickup_event(self: "OnistoneEssentials", ev: PlayerPickupItemEvent):
...
def handle_item_use(self: "OnistoneEssentials", ev: PlayerItemConsumeEvent):
    user = self.db.get_online_user(ev.player.xuid)
    if user is not None and (getattr(user, "is_jailed", None)):
        ev.is_cancelled = True
    return

def handle_item_drop_event(self: "OnistoneEssentials", ev: PlayerDropItemEvent):
    user = self.db.get_online_user(ev.player.xuid)
    if user is not None and (getattr(user, "is_jailed", None)):
        ev.is_cancelled = True
    return